...
public:
	//Constructor
	AStarSearch(graph_type* graph, int source, int target) : graph(graph), 
...
/************************************************************************/
template <class graph_type>
void AStarSearch<graph_type>::Search()
{
	...
	while (!pq.empty())
	{
		...
		for (graph_type::EdgeIterator edgeIt = graph->StartEdge(nextClosestNode); edgeIt != graph->EndEdge(nextClosestNode); edgeIt.next())
		{
			...
			if (graph->GetNode((*edgeIt).GetTo()).GetType() == NavType::unwalkable)
				continue;
...
template <class graph_type>
std::vector<int> AStarSearch<graph_type>::GetPathToTarget()
{
	std::vector<int> path;
