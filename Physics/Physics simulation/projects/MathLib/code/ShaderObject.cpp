...
namespace CGMath
{
	...
	void ShaderObject::setupShaders(const char* vertexFile, const char* fragmentFile)
	{
		...
		this->vShader = new GLchar[8192];
		this->fShader = new GLchar[8192];

		strncpy(this->vShader, vs, 8192);
		strncpy(this->fShader, fs, 8192);
	...
	void ShaderObject::setupMatrix3f(const GLchar* name, matrix3D mat)
	{
		std::string temp = name;
		if (uMap.find(temp) == uMap.end())
		{
    ...
    void ShaderObject::setupVector3f(const GLchar* name, vector3D vec)
    {
        ...
        this->useProgram();
        glUniform3f(uMap[temp], vec.x(), vec.y(), vec.z());
    }
	...
	GLchar* ShaderObject::GetVertexShader()
	{
		return vShader;
	}

	GLchar* ShaderObject::GetFragmentShader()
	{
		return fShader;
	}
