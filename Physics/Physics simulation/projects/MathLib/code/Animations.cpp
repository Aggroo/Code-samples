...
bool Animations::LoadAnimation(std::string filename)
{
	...
	if(header->numClips > 0)
	{
		...
		keyBuffer = new CGMath::Quaternion[header->numKeys];
		...
		int bufferSize = sizeof(CGMath::Quaternion)*header->numKeys;
		char* arr = new char[bufferSize];
		file.read(arr, bufferSize);
		memcpy(keyBuffer, (CGMath::Quaternion*)arr, 16*header->numKeys);
...
void Animations::PlayAnimation(int clipIndex, int keyIndex, CGMath::Skeleton* skel, float deltaTime)
{
	...
	for(int j = 0; j < curves[clipIndex].size(); j++)
	{
		...
		{
			posMat = CGMath::matrix4D::translationMatrix(CGMath::Quaternion::QuaternionToVec4(keyBuffer[curves[clipIndex][j].firstKeyIndex+keyIncrement*curves[clipIndex].size()]));
		}
